    ...
    public function getInfoRssNews($rssNews){
    	...
    	try{
	        ...
	        if(preg_match_all($reg_regular_item, $channelPage, $newsDetailInfo)) {
	        	$this->TNews->begin();
	        	for ($i=0; $i<count($newsDetailInfo[0]) && $i < 14; $i++) {
	        		
	        		if(!$this->getUpdateInfoRssNewsDetail($newsDetailInfo[1][$i], $rssNews)){
	        			break;
	        		}
	        	}
	        	$this->TNews->commit();
    	...
    	try{
    		...
    		if(preg_match_all($reg_regular_item, $channelPage, $newsDetailInfo)) {
    			...
    			for ($i=0; $i<count($newsDetailInfo[0]) && $i < 14; $i++) {
    				...
    				if(!$this->getUpdateInfoRssNewsDetailForAbcNews($newsDetailInfo[1][$i], $rssNews)){
    					break;
    				}
    			}
    			$this->TNews->commit();
...
/**
    ...
    public function getInfoRssNewsForJAAsahi($rssNews){
    	...
    	try{
    		...
    		if(preg_match_all($reg_regular_item, $channelPage, $newsDetailInfo)) {
    			$this->TNews->begin();
    			for ($i=1; $i<count($newsDetailInfo[0]) && $i < 14; $i++) {
    				 
    				if(!$this->getUpdateInfoRssNewsDetailForJAAsahi($newsDetailInfo[1][$i], $rssNews)){
    					break;
    				}
    			}
    			$this->TNews->commit();
	...
	function saveNews($pNews, $rssNews){
